...
/**
 * Unit tests for [ElisprsLexer]. These run under `./gradlew test` and
...
class ElisprsLexerTest {
    ...
    @Test fun `shebang on first line is its own token`() {
        val toks = nonWs("#!/usr/bin/env elisprs\necho 1\n")
        assertEquals(ElisprsTokenTypes.SHEBANG, toks[0].first)
    ...
    @Test fun `leading double-quote is a comment to end of line`() {
        ...
        assertTrue(toks.any { it.first == ElisprsTokenTypes.KEYWORD && it.second == "let" })
    }
    ...
    @Test fun `double-quote after a bar is a comment`() {
        ...
        assertTrue(
            ...
            toks.any { it.first == ElisprsTokenTypes.COMMENT && it.second.contains("trailing note") },
        )
        assertTrue(toks.any { it.first == ElisprsTokenTypes.BAR && it.second == "|" })
    }
    ...
    @Test fun `double-quote in expression position is a string`() {
        ...
        assertTrue(
            ...
            toks.any { it.first == ElisprsTokenTypes.STRING_DQ && it.second == "\"hello\"" },
        )
    }

    @Test fun `single-quoted string is literal with doubled-quote escape`() {
        val toks = nonWs("let s = 'it''s here'")
        val str = toks.first { it.first == ElisprsTokenTypes.STRING_SQ }
    ...
    @Test fun `control keywords classify as KEYWORD`() {
        for ((tt, _) in nonWs("if elseif else endif while endwhile for endfor return try catch finally endtry")) {
            assertEquals(ElisprsTokenTypes.KEYWORD, tt)
        }
    }
    ...
    @Test fun `ex commands classify as COMMAND`() {
        for ((tt, _) in nonWs("set setlocal autocmd augroup nnoremap highlight syntax source silent")) {
            assertEquals(ElisprsTokenTypes.COMMAND, tt)
        }
    }

    @Test fun `let assignment to a global scope var`() {
        val toks = nonWs("let g:loaded = 1")
        assertEquals(ElisprsTokenTypes.KEYWORD, toks[0].first)
        assertEquals(ElisprsTokenTypes.SCOPE_VAR, toks[1].first)
        assertEquals("g:loaded", toks[1].second)
        assertEquals(ElisprsTokenTypes.ASSIGN_OP, toks[2].first)
        assertEquals(ElisprsTokenTypes.NUMBER, toks[3].first)
    }

    @Test fun `scope prefixes all lex as one SCOPE_VAR token`() {
        for (v in listOf("g:x", "s:y", "b:z", "w:a", "t:b", "l:c", "a:000")) {
            val toks = nonWs(v)
            assertEquals("scope-var lookup failed for $v", ElisprsTokenTypes.SCOPE_VAR, toks[0].first)
    ...
    @Test fun `v-colon specials classify as SPECIAL_VAR`() {
        for (v in listOf("v:true", "v:false", "v:null", "v:count", "v:val", "v:shell_error", "v:exception")) {
            val toks = nonWs(v)
            assertEquals("special-var lookup failed for $v", ElisprsTokenTypes.SPECIAL_VAR, toks[0].first)
        }
    }
    ...
    @Test fun `v-colon non-special is a plain scope var`() {
        ...
        assertEquals(ElisprsTokenTypes.SCOPE_VAR, toks[0].first)
    }
    ...
    @Test fun `function declaration name colors as FUNCTION_DECL`() {
        ...
        assertTrue(
            ...
            toks.any { it.first == ElisprsTokenTypes.FUNCTION_DECL && it.second == "s:Greet" },
        )
    }
    ...
    @Test fun `unqualified function name colors as FUNCTION_DECL`() {
        ...
        assertTrue(
            "declared name should be FUNCTION_DECL: $toks",
            toks.any { it.first == ElisprsTokenTypes.FUNCTION_DECL && it.second == "Foo" },
        )
    }
    ...
    @Test fun `builtin function before paren classifies as BUILTIN_FUNCTION`() {
        ...
        assertTrue(
            ...
            toks.any { it.first == ElisprsTokenTypes.BUILTIN_FUNCTION && it.second == "len" },
        )
    }
    ...
    @Test fun `function used as funcref builtin is not a declaration`() {
        ...
        assertTrue(
            ...
            toks.any { it.first == ElisprsTokenTypes.BUILTIN_FUNCTION && it.second == "function" },
        )
    }
    ...
    @Test fun `builtin name not followed by paren stays identifier`() {
        ...
        assertTrue(
            ...
            toks.any { it.first == ElisprsTokenTypes.IDENTIFIER && it.second == "len" },
        )
    }
    ...
    @Test fun `autoload call colors the qualified name as FUNCTION_DECL`() {
        ...
        assertTrue(
            ...
            toks.any { it.first == ElisprsTokenTypes.FUNCTION_DECL && it.second == "plug#begin" },
        )
    }
    ...
    @Test fun `option reference lexes as OPTION`() {
        for (o in listOf("&number", "&l:textwidth", "&g:foldlevel")) {
            val toks = nonWs(o)
            assertEquals("option lookup failed for $o", ElisprsTokenTypes.OPTION, toks[0].first)
    ...
    @Test fun `environment variable lexes as ENV_VAR`() {
        ...
        assertTrue(
            ...
            toks.any { it.first == ElisprsTokenTypes.ENV_VAR && it.second == "\$HOME" },
        )
    }
    ...
    @Test fun `register reference lexes as REGISTER`() {
        ...
        assertTrue(
            ...
            toks.any { it.first == ElisprsTokenTypes.REGISTER && it.second == "@a" },
        )
    }
    ...
    @Test fun `numbers decimal hex binary and float`() {
        ...
        assertEquals(ElisprsTokenTypes.NUMBER, nonWs("0x1F")[0].first)
        assertEquals(ElisprsTokenTypes.NUMBER, nonWs("0b1010")[0].first)
    ...
    @Test fun `bar is its own token and logical-or is operator`() {
        ...
        assertTrue("expected BAR: $toks", toks.any { it.first == ElisprsTokenTypes.BAR && it.second == "|" })
        val toks2 = nonWs("if a || b")
        assertTrue("expected || OPERATOR: $toks2", toks2.any { it.first == ElisprsTokenTypes.OPERATOR && it.second == "||" })
    }

    @Test fun `comparison operators with case flags`() {
        val toks = nonWs("if a ==# b && c =~? d")
        val ops = toks.filter { it.first == ElisprsTokenTypes.OPERATOR }.map { it.second }
    ...
    @Test fun `string concat dot-dot is an operator`() {
        ...
        assertTrue(
            ...
            toks.any { it.first == ElisprsTokenTypes.OPERATOR && it.second == ".." },
        )
    }
    ...
    @Test fun `compound assignment operators`() {
        for (op in listOf("+=", "-=", ".=")) {
            ...
            assertTrue(
                ...
                toks.any { it.first == ElisprsTokenTypes.ASSIGN_OP && it.second == op },
            )
        }
    }
    ...
    @Test fun `leading backslash is a line continuation`() {
        val src = "let l = [\n\\ 1,\n\\ 2,\n\\ ]\n"
        val toks = tokens(src)
        val conts = toks.filter { it.first == ElisprsTokenTypes.LINE_CONTINUATION }
    ...
    @Test fun `brackets lex as distinct L and R tokens`() {
        ...
        assertTrue(types.contains(ElisprsTokenTypes.LBRACE))
        assertTrue(types.contains(ElisprsTokenTypes.RBRACE))
        assertTrue(types.contains(ElisprsTokenTypes.LBRACKET))
        assertTrue(types.contains(ElisprsTokenTypes.RBRACKET))
        assertTrue(types.contains(ElisprsTokenTypes.COMMA))
    }
}
