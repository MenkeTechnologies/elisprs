...
/**
 * [XDebugProcess] speaking DAP over the `elisprs --dap` server's stdio.
 *
 * Unlike a TCP adapter, elisprs multiplexes the debuggee's own `:echo` /
...
class ElisprsDebugProcess(
    ...
    @Volatile var client: ElisprsDapClient? = null
        private set

    private val executionStack = ElisprsExecutionStack()
    private val editorsProvider = ElisprsDebuggerEditorsProvider()
    private val breakpointHandlers = arrayOf<XBreakpointHandler<*>>(ElisprsBreakpointHandler(this))
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                c.request(
                    "initialize",
                    JsonObject().apply {
                        addProperty("clientID", "intellij-elisprs")
                        addProperty("clientName", "IntelliJ elisprs")
                        addProperty("adapterID", "elisprs")
    ...
    private fun sendAllBreakpoints() {
        ...
        for (bp in mgr.getBreakpoints(ElisprsBreakpointType::class.java)) {
    ...
    private fun onStopped(body: JsonObject) {
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                ...
                val builtFrames = mutableListOf<ElisprsStackFrame>()
                for (rf in rawFrames) {
                    ...
                    val children = mutableListOf<ElisprsValue>()
                    if (scopes != null) {
                        for (s in scopes) {
                ...
                val ctx = ElisprsSuspendContext(executionStack)
                ApplicationManager.getApplication().invokeLater {
    ...
    companion object {
        private val LOG = Logger.getInstance(ElisprsDebugProcess::class.java)
    }
}
