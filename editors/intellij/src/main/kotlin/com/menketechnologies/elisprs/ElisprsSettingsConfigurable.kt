...
class ElisprsSettingsConfigurable : Configurable {

    private val executableField = TextFieldWithBrowseButton().apply {
        addBrowseFolderListener(
            "elisprs Executable",
            "Path to the elisprs binary",
            null,
            FileChooserDescriptorFactory.createSingleFileNoJarsDescriptor(),
        )
    }
    private val lspEnabledBox = JBCheckBox("Enable LSP (uses `elisprs --lsp`)")
    ...
    override fun createComponent(): JComponent {
        val p = FormBuilder.createFormBuilder()
            ...
            .addLabeledComponent(JBLabel("elisprs executable:"), executableField, 1, false)
            .addTooltip("Leave blank to use the first `elisprs` on \$PATH.")
    ...
    override fun isModified(): Boolean {
        val s = ElisprsSettings.getInstance()
        return executableField.text != (s.elisprsExecutable ?: "") ||
    ...
    override fun apply() {
        val s = ElisprsSettings.getInstance()
        s.elisprsExecutable = executableField.text.takeIf { it.isNotBlank() }
    ...
    override fun reset() {
        val s = ElisprsSettings.getInstance()
        executableField.text = s.elisprsExecutable ?: ""
        lspEnabledBox.isSelected = s.lspEnabled
        extraLspArgsField.text = s.extraLspArgs
        disableLexerBox.isSelected = s.disableLexerHighlighting
        fileExtensionsField.text = s.fileExtensions
