package com.menketechnologies.elisprs.refactor
...
import com.menketechnologies.elisprs.lsp.ElisprsLspServerSupportProvider
 ...
 * actions returned by `elisprs --lsp`.
 *
 ...
 * `elisprs/src/extensions/lsp.rs::code_actions` (`Extract to variable
 * (…)` / `Extract to constant (…)`). If those titles change, update
 * the [titleMatches] predicates below.
 *
 * The elisprs LSP server emits Extract Variable, Extract Constant, and
 ...
 */
class ElisprsRefactoringSupportProvider : RefactoringSupportProvider() {
    ...
    override fun getIntroduceParameterHandler(): RefactoringActionHandler =
        LspExtractActionHandler(
            ...
            hint = "elisprs LSP does not emit Extract Parameter actions. Use Cmd-Opt-V to extract to a `local NAME=…` instead.",
        )
}
...
private class LspExtractActionHandler(
    ...
    override fun invoke(
        ...
        val server = findElisprsLspServer(project)
        if (server == null) {
            dbg("ABORT: no LSP server found via LspServerManager.getServersForProvider(ElisprsLspServerSupportProvider::class.java)")
            notifyUser(project, "$refactoringName: LSP server not running. Check Help → Show Log for `Starting elisprs LSP:`.")
            return
        }
    ...
    private fun dbg(msg: String) {
        com.menketechnologies.elisprs.ElisprsDebugLog.log("refactor:$refactoringName", msg)
    }
    ...
    private fun findElisprsLspServer(project: Project): LspServer? =
        LspServerManager.getInstance(project)
            .getServersForProvider(ElisprsLspServerSupportProvider::class.java)
            .firstOrNull()
    ...
    private fun notifyUser(project: Project, message: String) {
        ...
        val group = NotificationGroupManager.getInstance()
            .getNotificationGroup("elisprs Refactoring")
