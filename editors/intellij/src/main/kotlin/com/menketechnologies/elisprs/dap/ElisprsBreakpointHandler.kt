...
class ElisprsBreakpointHandler(
    private val process: ElisprsDebugProcess,
) : XBreakpointHandler<XLineBreakpoint<XBreakpointProperties<*>>>(ElisprsBreakpointType::class.java) {
    ...
    private fun resync(fileUrl: String) {
        ...
        val bps: List<Int> = ReadAction.compute<List<Int>, RuntimeException> {
            val all = com.intellij.xdebugger.XDebuggerManager.getInstance(process.session.project)
                .breakpointManager
                .getBreakpoints(ElisprsBreakpointType::class.java)
