 ...
 *
 * Strykelang doesn't have this (yet); elisprs adds it because shell
 * scripts use `{ … }` and `( … )` heavily — duplicate-close was the
 * single most-reported lexer/editor papercut.
 */
class ElisprsTypedHandler : TypedHandlerDelegate() {
    override fun beforeCharTyped(
        ...
        if (file.fileType !is ElisprsFileType) return Result.CONTINUE
