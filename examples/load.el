...
(ert-deftest load-runs-file-and-binds-vars ()
  ...
  ;; At top level (a script run, not inside a load) the load vars are nil.
  (should (null load-file-name))
  (should (null load-in-progress))
  (let ((r (load (loadtest-fixture "helper.el"))))
    ...
    ;; …and both are unwound back to nil after `load' returns.
    (should (null load-file-name))
    (should (null load-in-progress))))
