...
pub const SUBRS: &[Entry] = &[
    ...
    Entry {
        ...
        doc: "Width of STRING in pixels: display columns, with TAB advancing to the next tab stop.",
    },
    Entry {
        name: "define-hash-table-test",
        kind: Kind::Function,
        sig: "(define-hash-table-test NAME TEST HASH)",
        doc: "Define a hash table test NAME whose elisp TEST compares keys and HASH hashes them.",
    },
    Entry {
        name: "text-property-any",
        kind: Kind::Function,
        sig: "(text-property-any START END PROP VALUE &optional OBJECT)",
        doc: "First position in [START, END) where PROP is `eq' to VALUE, or nil.",
    },
    Entry {
        name: "text-property-not-all",
        kind: Kind::Function,
        sig: "(text-property-not-all START END PROP VALUE &optional OBJECT)",
        doc: "First position in [START, END) where PROP is not `eq' to VALUE, or nil.",
    },
    Entry {
        name: "listify-key-sequence",
        kind: Kind::Function,
        sig: "(listify-key-sequence KEY)",
        doc: "Convert the key sequence KEY to a list of events.",
    },
    Entry {
        name: "looking-back",
        kind: Kind::Function,
        sig: "(looking-back REGEXP &optional LIMIT GREEDY)",
        doc: "Non-nil if the text before point matches REGEXP.",
    },
    Entry {
        name: "regexp-opt",
        kind: Kind::Function,
        sig: "(regexp-opt STRINGS &optional PAREN)",
        doc: "Regexp matching any of STRINGS, with shared affixes factored out.",
    },
    Entry {
        name: "regexp-opt-charset",
        kind: Kind::Function,
        sig: "(regexp-opt-charset CHARS)",
        doc: "Regexp matching one character in CHARS, using ranges where they pay.",
    },
    Entry {
        name: "pcase-lambda",
        kind: Kind::Function,
        sig: "(pcase-lambda LAMBDA-LIST &rest BODY)",
        doc: "A `lambda' whose parameters may be pcase patterns.",
    },
    Entry {
        name: "symbol-name",
        kind: Kind::Function,
